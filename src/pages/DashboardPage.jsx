...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchDashboardData();
    // 의존성 배열에 syncStatus를 넣지 않아도 됨(함수형 업데이트 사용 중)
  }, []);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8">
        ...
        <Link
          to="/settings"
          className="px-4 py-2 mt-4 md:mt-0 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500"
        >
          설정
        </Link>
      </div>
            ...
            <>
              <div className="flex items-center mb-4">
                <svg className="w-6 h-6 mr-2" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fillRule="evenodd"
                    d="M10 0C4.477 0 0 4.477 0 10c0 4.42 2.865 8.166 6.839 
                    9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.341-3.369-1.341
                    -.454-1.155-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531
                    1.03.892 1.529 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943
                    0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.272.098-2.65 0 0 .84-.269 
                    2.75 1.025A9.564 9.564 0 0110 2.836c.85.004 1.705.115 2.504.337 1.909-1.294
                    2.747-1.025 2.747-1.025.546 1.378.203 2.397.1 2.65.64.699 1.028 
                    1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 
                    4.933.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012
                    2.743 0 .267.18.578.688.48C17.137 18.163 20 14.417
                    20 10c0-5.523-4.477-10-10-10z"
                    clipRule="evenodd"
                  />
                </svg>
                <a
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-3xl font-bold text-green-600">
            {syncStatus.isLoading
              ? '...'
              : recentSubmissions.filter((s) => s.result === 'AC').length}
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-3xl font-bold text-purple-600">
            {syncStatus.isLoading
              ? '...'
              : recentSubmissions.filter((s) => s.synced).length}
          </p>
        </div>
      </div>
    </div>
